;(function ($, dialogPolyfill, PointRecord) {
  ...
  function editPointRecord (pointRecord) {
    _inputPointRecordId.value(pointRecord._id === 0 ? '0' : pointRecord._id)
  ...
  function configDialog () {
    _buttonNewPoint.on('click', () => {
      _inputPointRecordId.element.value = ''
      _inputEntryTime.element.value = ''
      _inputLunchTime.element.value = ''
      _inputReturnTime.element.value = ''
      _inputExitTime.element.value = ''
      _dialog.showModal()
    })
